...
func NewServer(ctx context.Context, cfg *config.Config,logger *logrus.Logger) (*Server, error) {
	...
	r.HandleFunc("/put/", srv.putWithCommitmentHandler).Methods("POST")
	r.HandleFunc("/get/", srv.getCommitmentHandler).Methods("GET")
...
func (s *Server) putWithoutCommitmentHandler(w http.ResponseWriter, r *http.Request)  {
	...
	storageManager, err := contract.NewStorageManager(common.HexToAddress(storageManagerAddress), s.SenderClient)
	if err != nil {
		s.log.Fatalf("NewStorageManager----failed error:%s", err)
		http.Error(w, "NewStorageManager failed", http.StatusBadRequest)
		return
	}

	cmManager, err := contract.NewCommitmentManager(common.HexToAddress(cmManagerAddress), s.SenderClient)
	if err != nil {
		s.log.Fatalf("NewCommitmentManager----failed error:%s", err)
		http.Error(w, "NewCommitmentManager failed", http.StatusBadRequest)
		return
	}

	nodeManager, err := contract.NewNodeManager(common.HexToAddress(nodeManagerAddress), s.SenderClient)
	if err != nil {
		s.log.Fatalf("NewNodeManager----failed error:%s", err)
		http.Error(w, "NewNodeManager failed", http.StatusBadRequest)
		return
	}
	...
	sender := s.auth.From
	index, err := cmManager.Indices(nil, sender)
	length := len(data)
	...
	for _, add := range nodeGroup.Addrs {
		...
		result,err := s.getSignature(sender,index.Uint64(),uint64(length),cm.Marshal(),data,nodeGroupKey,proof.H.Marshal(),claimedValue,uint64(timeOut),[]byte{})
		if err != nil {
	...
	tx, err := cmManager.SubmitCommitment(s.auth,new(big.Int).SetInt64(int64(length)),new(big.Int).SetInt64(timeOut),nameSpaceKey,nodeGroupKey,signatures,commitData)
	if err != nil {
	...
	if receipt.Status == types.ReceiptStatusFailed {
		...
		s.log.Printf("cm string:%s orig:%v",common.Bytes2Hex(cm.Marshal()),cm.Marshal())
...
func (s *Server) putWithCommitmentHandler(w http.ResponseWriter, r *http.Request) {
	...
	commitment := strings.TrimPrefix(r.URL.String(), "/put/?=")
	if len(commitment) == 0 {
		commitment = strings.TrimPrefix(r.URL.String(), "/put/?")
		if len(commitment) == 0 {
	...
	commitmentByte := common.Hex2Bytes(commitment)
	...
	if err != nil {
		s.log.Fatalf("NewCommitmentManager----failed error:%s", err)
		http.Error(w, "NewCommitmentManager Error", http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		s.log.Fatalf("NewNodeManager----failed error:%s", err)
		http.Error(w, "NewNodeManager Error", http.StatusBadRequest)
		return
	}
	nodeGroupKey := common.HexToHash(s.config.NodeGroup)
	nameSpaceKey := common.HexToHash(s.config.Namespace)
	...
	index, err := cmManager.Indices(nil, sender)
	length := len(preimage)
	s.log.Printf("transitStation is prepare getting signature data:%s commitment:%s",common.Bytes2Hex(preimage),commitment)
	for _, add := range nodeGroup.Addrs {
		info, err := nodeManager.BroadcastingNodes(nil, add)
		if err != nil {
			continue
...
func (s *Server) getCommitmentHandler(w http.ResponseWriter, r *http.Request) {
	commitment := strings.TrimPrefix(r.URL.String(), "/get/?=")
	if len(commitment) == 0 {
		commitment = strings.TrimPrefix(r.URL.String(), "/get/?")
		if len(commitment) == 0 {
			s.log.Println("Get commitment params is missing")
	...
	commitmentData := common.Hex2Bytes(commitment)
	storageManager, err := contract.NewStorageManager(common.HexToAddress(storageManagerAddress), s.SenderClient)
	if err != nil {
		log.Fatalf("NewStorageManager----failed error:%s", err)
		http.Error(w, "NewStorageManager failed", http.StatusBadRequest)
		return
	}
	nodeManager, err := contract.NewNodeManager(common.HexToAddress(nodeManagerAddress), s.SenderClient)
	...
	for _, add := range nodeGroup.Addrs {
		...
		if daGetErr != nil || &daDetail == nil{
			daGetErr = s.SignerClient.Client().CallContext(s.ctx, &daDetail, "mta_getDAByCommitment", commitment)
			if daGetErr != nil || &daDetail == nil{
